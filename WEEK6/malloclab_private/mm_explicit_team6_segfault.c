...
static void place(void *bp, size_t asize)   
{
    ...
    {    
        ...
        PUT(PREC(bp), last_freep);  //현재의 predecessor에 이전 last_
        PUT(SUCC(bp), NULL);        //내가 가용리스트의 마지막에 들어갈 거라서

        PUT(SUCC(last_freep),bp);   //앞의 successor와 연결
        last_freep = bp;
    }
    else                                    
    {
