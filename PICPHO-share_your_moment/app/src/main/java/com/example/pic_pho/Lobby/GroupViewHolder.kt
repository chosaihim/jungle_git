...
class GroupViewHolder(itemView: View):RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(groupModel: GroupModel){
        ...
        for(i in 0 .. 1) {
            var queryFriends = "SELECT userProfileImage FROM Friends WHERE userUid = ${arrayMemberUidList[i]}"
        ...
        if(memberCount > 2){
            var queryFriends = "SELECT userProfileImage FROM Friends WHERE userUid = ${arrayMemberUidList[2]}"
            var cursor = database.rawQuery(queryFriends, null)
            cursor.moveToFirst()
            memberUriList.add(cursor.getString(0))
            Log.e(TAG, "GroupBiewHolder에서 이미지 테스트: ${cursor.getString(0)}")
