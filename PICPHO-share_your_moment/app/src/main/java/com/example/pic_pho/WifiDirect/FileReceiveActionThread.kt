    ...
    override fun run() {
        ...
        when (String(protocolByteArray, Charsets.UTF_8)) {
            ...
            "1" -> {
                ...
                Log.d(
                    TAG,
                    "FileReceiveActionThread 접속 중인 피어 수 : ${WifiDirectMainActivity.connectedPeerList.size}"
                )
            }
...
//                            "Member : ${WifiDirectMainActivity.groupMemberCount}" + "Sent : ${WifiDirectMainActivity.sentPhotoCount}"
            ...
            "3" -> {
                Log.d(
                    TAG,
                    "run: Server에서 프로토콜 3 받음, Photolist size = ${WifiDirectMainActivity.photoInfoList.size}"
                )
                var intent = Intent(context, PhotoRoomActivity::class.java)
                context.startActivity(intent)
                sleep(1000)
